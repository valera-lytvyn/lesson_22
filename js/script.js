...
function three_digit_number() {
   ...
   if (threeDigitNumber < 100 || threeDigitNumber > 999) {
      ...
      alert(`Ваше число ${threeDigitNumber} містить три одинакові цифри ${figure_1}`);
   } else if (figure_1 == figure_2) {
      alert(`Ваше число ${threeDigitNumber} містить дві одинакові цифри ${figure_1}`);
   } else if (figure_2 == figure_3) {
            alert(`Ваше число ${threeDigitNumber} містить дві одинакові цифри ${figure_2}`);
         } else if (figure_1 == figure_3) {
            alert(`Ваше число ${threeDigitNumber} містить дві одинакові цифри ${figure_3}`);
   } else if (figure_1 == figure_2 == figure_3) {
      alert(`Ваше число ${threeDigitNumber} містить три одинакові цифри ${figure_3}`);
   }            else {
...
function year() {
   ...
   } else  if (chooseYear % 4 == 0 || chooseYear % 400 == 0 && chooseYear % 100 > 0) {
...
function discount() {
   ...
   if(discount >= 500) {
   alert(`Ваша покупка на суму ${discount}, знижка 7%, зі знижкою складає ${discount-discount*.07}`)
} else if (discount >= 300) {
   alert(`Ваша покупка на суму ${discount}, знижка 5%,  зі знижкою складає ${discount-discount*.05}`)
} else if (discount >= 200) {
   alert(`Ваша покупка на суму ${discount}, знижка 3%, зі знижкою складає ${discount-discount*.03}`)
} else  if (discount < 200) {
...
function circle() {
   ...
   if (circleLenght / Math.PI < perimeterSquare / 4) {
      alert(`Коло довжиною ${circleLenght} зможе поміститись в квадрат з периметром ${perimeterSquare} `)
   } else {
           alert(`Коло довжиною ${circleLenght} не зможе поміститись в квадрат з периметром ${perimeterSquare} `)
   }
}
...
function any_date() {
   ...
   if ((anyMonth < 0 || anyMonth >= 13 || ((anyMonth == 4 || anyMonth == 6 || anyMonth == 9 || anyMonth == 11) && anyDay > 30)) || anyDay > 31) {
      alert("День або місяць введені невірно")
   } else 
      if (anyYear % 4 == 0 || anyYear % 400 == 0) {
         if (anyYear % 100 > 0 && anyMonth == 2 && anyDay == 29) {
            alert(`01.03.${anyYear}`)
               }
   } else 
   if (anyMonth == 2 && anyDay > 28) {
   ...
   if (anyMonth == 12 && anyDay == 31) {
      alert(`01.01.${++anyYear}`)
   } else
   if ((anyMonth == 1 || anyMonth == 3 || anyMonth == 5 || anyMonth == 7 || anyMonth == 8 || anyMonth == 10) && anyDay == 31) {
      alert(`01.${++anyMonth}.${anyYear}`)
   } else
   if ((anyMonth == 4 || anyMonth == 6 || anyMonth == 9 || anyMonth == 11) && anyDay == 30) {
      alert(`01.${++anyMonth}.${anyYear}`)
   } else {
...
function currency_converter() {
   ...
   let currency = prompt("Введіть валюту, в яку потрібно перевести, якщо EUR натисніть 1, UAN - 2, AZN - 3 ");
   if (currency < 1 || currency > 3) {
      alert(`Конвертер підтримує тільки три валюти EUR - 1, UAN - 2, AZN - 3`)
   } else   if (currency == 1) {
      alert(`Введена сума ${dollarAmount}$ при конвертаціїї в EUR ,буде рівна ${(dollarAmount * .98).toFixed(2)} `)
   } else if (currency == 2) {
      alert(`Введена сума ${dollarAmount}$ при конвертаціїї в UAN ,буде рівна ${(dollarAmount * 36.5).toFixed(2)} `)
   } else if (currency == 3) {
      alert(`Введена сума ${dollarAmount}$ при конвертаціїї в AZN ,буде рівна ${(dollarAmount * 1.7).toFixed(2)} `)
   }
}

function questions() {
   alert(`За кожну правильну відповідь Вам нарахується 2бали`);
   let question_1 = prompt("Хто розпочав війну 24.02.22? Україна - 1, НАТО - 2, Московщина -3");
   ...
   let question_2 = prompt("Хто переможе в цій війні? Україна - 1, НАТО - 2, Московщина -3");
   if (question_2 == 1) {
      amountForAnswers = amountForAnswers + 2;
      alert("Вірно! 2бали")
   } else {
